...
type AppView = 'home' | 'dashboard' | 'predictor' | 'graph';
...
function PredictorWorkbench() {
  ...
  const applySafeFix = (victim: number) => {
    const newAllocation = allocation.map((row, i) => (i === victim ? row.map(() => 0) : [...row]));
    const newMax = max.map((row, i) => (i === victim ? row.map(() => 0) : [...row]));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await localApiFetch('/api/predict', {
        method: 'POST',
        body: JSON.stringify({
          processes,
          resources,
                          ...
                          <td key={j}>
                            <input
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                handleMatrixChange(setAllocation, allocation, i, j, e.target.value === '' ? 0 : Number(e.target.value))
                              }
                        ...
                        {row.map((val: number, j: number) => (
                          <td key={j}>
                            <input
                              ...
                              value={val === 0 ? '' : val}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                handleMatrixChange(setMax, max, i, j, e.target.value === '' ? 0 : Number(e.target.value))
                              }
          ...
          {step === 4 && (
            <div className="fade-in space-y-10">
              ...
              <div className="pt-6 border-t border-slate-200/50">
                ...
                >
                  {loading ? <span className="w-6 h-6 border-4 border-slate-300 border-t-slate-600 rounded-full animate-spin"></span> : <>⚡ Run Prediction ⚡</>}
                </button>
              </div>
                    ...
                    <div className="mt-8 p-6 skeuo-pressed rounded-3xl border border-blue-100/50 bg-blue-50/30">
                      ...
                      <div className="flex flex-wrap justify-center gap-4">
                        {result.logical_deadlock ? (
                          <button
                            onClick={() => {
                              const victimId = parseInt(result.recommended_action?.match(/Process (\d+)/)?.[1] || '-1', 10);
                              if (victimId !== -1) {
      ...
      <aside className="lg:col-span-4 space-y-8 fade-in" style={{ animationDelay: '0.2s' }}>
        <div className="glass-card rounded-[2rem] p-8 shadow-xl">
          ...
          <div className="space-y-6">
            {[
              ...
              { title: 'Simulation Engine', status: 'Active', desc: 'Real-time resource flow' },
              { title: 'Visual RAG', status: 'Active', desc: 'Graph-based visualization' },
            ].map((item, idx) => (
              <div key={idx} className="p-4 rounded-2xl transition-all skeuo-pressed border-l-4 border-blue-500">
        ...
        <div className="skeuo-flat rounded-[2rem] p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {['Frontend Routes', 'TypeScript', 'React 19', 'Vite'].map((tech) => (
...
function App() {
  ...
  const navItemClass = (view: AppView) =>
    `px-4 py-2 rounded-xl text-sm font-bold transition-all ${
      activeView === view ? 'bg-blue-600 text-white shadow-lg' : 'skeuo-pressed text-slate-600 hover:text-blue-600'
    }`;

  return (
    <div className="min-h-screen bg-[#e0e5ec] text-slate-700 py-6 px-4 md:px-8">
      <header className="max-w-7xl mx-auto mb-8 fade-in">
        <div className="glass-card rounded-[2rem] px-5 md:px-8 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-slate-800 uppercase">
              Safe<span className="text-blue-600">Sched</span>
            </h1>
            <p className="text-sm md:text-base text-slate-500">
              Industry-style deadlock control web app: scenario, simulation, graph, recovery, and reports.
            </p>
          </div>
          <nav className="flex flex-wrap justify-center gap-2 md:gap-3">
            <button onClick={() => setActiveView('home')} className={navItemClass('home')}>Overview</button>
            <button onClick={() => setActiveView('dashboard')} className={navItemClass('dashboard')}>Dashboard</button>
            <button onClick={() => setActiveView('graph')} className={navItemClass('graph')}>Graphs</button>
            <button onClick={() => setActiveView('predictor')} className={navItemClass('predictor')}>Analysis</button>
          </nav>
        </div>
      </header>
      ...
      {activeView === 'home' && (
        <main className="max-w-7xl mx-auto space-y-8">
          <section className="glass-card rounded-[2.5rem] p-8 md:p-12 shadow-xl">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-7">
                <div className="skeuo-pressed inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold text-blue-600 uppercase tracking-wider mb-5">
                  Real-time Resource Safety Platform
                </div>
                <h2 className="text-3xl md:text-5xl font-black tracking-tight text-slate-800 leading-tight">
                  Predict, visualize, and resolve deadlocks before they impact your system.
                </h2>
                <p className="mt-5 text-slate-500 text-base md:text-lg leading-relaxed max-w-2xl">
                  SafeSched combines scenario modeling, live simulation controls, graph intelligence, and AI risk analysis in one modern workspace for scheduling and concurrency operations.
                </p>
                <div className="mt-7 flex flex-wrap gap-3">
                  <button
                    onClick={() => setActiveView('dashboard')}
                    className="px-6 py-3 rounded-xl bg-blue-600 text-white font-bold shadow-[0_10px_30px_-10px_rgba(37,99,235,0.6)] hover:scale-[1.02] active:scale-95 transition-all"
                  >
                    Launch Dashboard
                  </button>
                  <button
                    onClick={() => setActiveView('predictor')}
                    className="px-6 py-3 rounded-xl skeuo-pressed font-bold text-slate-700 hover:text-blue-600 transition-all"
                  >
                    Run Deadlock Analysis
                  </button>
                </div>
                <div className="mt-6 flex flex-wrap gap-2">
                  {['Banker-style safety checks', 'Interactive graph insights', 'Recovery-oriented workflows'].map((item) => (
                    <span key={item} className="px-3 py-1 rounded-full text-xs font-bold skeuo-pressed text-slate-600">
                      {item}
                    </span>
                  ))}
                </div>
              ...
              <div className="lg:col-span-5">
                <div className="skeuo-flat rounded-[2rem] p-6">
                  <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest mb-4">Quick Access</h3>
                  <div className="space-y-3">
                    {[
                      { title: 'Simulation Dashboard', desc: 'Control playback, edit scenarios, monitor live KPIs.', to: 'dashboard' as AppView, cta: 'Open Dashboard' },
                      { title: 'Graphs Workspace', desc: 'Inspect process-resource relationships and potential cycles.', to: 'graph' as AppView, cta: 'Open Graphs' },
                      { title: 'AI Analysis Engine', desc: 'Evaluate allocation/max/available states and risk confidence.', to: 'predictor' as AppView, cta: 'Open Analysis' },
                    ].map((item) => (
                      <button
                        key={item.title}
                        onClick={() => setActiveView(item.to)}
                        className="w-full text-left glass-card rounded-2xl p-4 hover:scale-[1.01] transition-all"
                      >
                        <div className="font-bold text-slate-800 text-sm">{item.title}</div>
                        <div className="text-xs text-slate-500 mt-1">{item.desc}</div>
                        <div className="text-xs text-blue-600 font-bold mt-2">{item.cta} →</div>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {[
              {
                ...
                points: ['Matrix grid editor', 'Validation warnings', 'Presets + random generator', 'JSON import/export'],
              },
              {
                title: 'Live Simulation',
                points: ['Step + auto-run controls', 'Event-progress visibility', 'Queue-oriented analysis', 'Operational quick actions'],
              },
              {
                title: 'Recovery & Reports',
                points: ['Deadlock process targeting', 'Suggested intervention path', 'Recovery tracking signals', 'Report-friendly outputs'],
              },
            ].map((block) => (
              <article key={block.title} className="glass-card rounded-[2rem] p-6 shadow-lg">
                <h3 className="text-lg font-black text-slate-800">{block.title}</h3>
                <ul className="mt-4 space-y-2">
                  {block.points.map((point) => (
                    <li key={point} className="text-sm text-slate-500 flex items-start gap-2">
      ...
      {activeView === 'graph' && (
        <main className="max-w-6xl mx-auto glass-card rounded-[2rem] p-6 md:p-10 shadow-xl">
          <div className="flex items-center justify-between mb-6 gap-4">
            <h2 className="text-2xl font-bold text-slate-800">Graph Visualizer</h2>
            <button onClick={() => setActiveView('dashboard')} className="skeuo-button px-4 py-2 rounded-xl">
              Open Full Dashboard
            </button>
          </div>
          <GraphVisualizer />
      ...
      {activeView === 'predictor' && <PredictorWorkbench />}
    </div>
  );
}
